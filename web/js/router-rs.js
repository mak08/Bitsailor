...
( function () {
    ...
    async function computePath (event) {
        if ( Router.twaAnchor ) {
            ...
            let wind = await Router.gribCache.getWind(slat, slon, startTime);
            ...
            if (twa != curTWA) {
                ...
                for (var step = step0; step < 86400 && pathDist < targetDist; step += 600) {
                    ...
                    let windTWA = await Router.gribCache.getWind(newTWAPos.lat, newTWAPos.lon, stepTime);
                    let windHDG = await Router.gribCache.getWind(newHDGPos.lat, newHDGPos.lon, stepTime);
    ...
    function setUpRS () {
        ...
        Router.setSailnames(["Jib", "Gennaker", "Spi"]);
